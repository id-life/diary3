...
export const createEntryTypeDbFirstAtom = atom(null, (get, set, entryType: EntryType) => {
  const current = get(hybridEntryTypesAtom);
  set(hybridEntryTypesAtom, [...current, entryType]);
});

export const updateEntryTypeDbFirstAtom = atom(null, (get, set, entryType: EntryType) => {
  const current = get(hybridEntryTypesAtom);
...
export const deleteEntryTypeDbFirstAtom = atom(null, (get, set, entryTypeId: string) => {
  const current = get(hybridEntryTypesAtom);
...
export const migrateFromLocalStorageAtom = atom(null, (get, set) => {
  console.log('🔄 Migrating from localStorage to memory atoms...');

  try {
    if (typeof window === 'undefined') {
      console.warn('Migration attempted on server-side, skipping...');
      return false;
    }
